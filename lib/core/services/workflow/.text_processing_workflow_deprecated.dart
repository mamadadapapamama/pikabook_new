  ...
  Future<ProcessedText> processText({
    ...
    try {
      ...
      if (!effectiveSegmentMode) {
        ...
        // 번역 캐시 확인
        final cacheKey = '${note.sourceLanguage}:${note.targetLanguage}:$text';
        ...
        if (segments.isNotEmpty) {
          ...
          final cachedTranslations = <int, String>{};
          ...
          if (kDebugMode) {
            debugPrint('캐시 히트: ${cachedTranslations.length}개, 번역 필요: ${segmentsToTranslate.length}개');
          }
          
          // 번역 결과 저장용 배열
          final segmentTranslations = List<String>.filled(segments.length, '');
          
          // 캐시된 번역 결과 적용
          for (final entry in cachedTranslations.entries) {
            segmentTranslations[entry.key] = entry.value;
          }
          
          // 캐시에 없는 세그먼트만 번역
          if (segmentsToTranslate.isNotEmpty) {
            if (kDebugMode) {
              debugPrint('세그먼트 번역 시작: ${segmentsToTranslate.length}개');
            }
            ...
            for (final batch in segmentBatches) {
              ...
              for (int i = 0; i < batch.length; i++) {
                if (i < translationResult.length) {
                  ...
                  // 번역 결과 캐싱 (개별 세그먼트)
                  final originalText = segmentsToTranslate[segmentIdx]!;
                  final translation = translationResult[i];
                  if (translation.isNotEmpty) {
                    final cacheKey = '${note.sourceLanguage}:${note.targetLanguage}:$originalText';
                    await _cacheService.setTranslationCache(cacheKey, translation);
                  }
                }
              }
            }
          ...
          // 전체 번역 결과도 캐싱 (다음에 전체 모드로 전환 시 사용)
          if (translatedText.isNotEmpty) {
            final fullCacheKey = '${note.sourceLanguage}:${note.targetLanguage}:$text';
            await _cacheService.setTranslationCache(fullCacheKey, translatedText);
          }
  ...
  Future<List<String>> _batchTranslate(
    ...
    try {
      ...
      if (uncachedTexts.isNotEmpty) {
        if (kDebugMode) {
          debugPrint('배치 번역 필요: ${uncachedTexts.length}개 세그먼트');
        }
        ...
        // 배치 내 텍스트 결합
        final combinedText = uncachedTexts.join('\n$uniqueMarker\n');
        ...
        // 번역 결과 분리
        final translations = combinedTranslation.split(uniqueMarker)
            .map((t) => t.trim())
            .toList();
        ...
        for (int i = 0; i < uncachedIndices.length; i++) {
          if (i < translations.length) {
            ...
            final originalText = texts[originalIndex];
            ...
            // 번역 결과 캐싱
            if (translation.isNotEmpty) {
              final cacheKey = '$sourceLanguage:$targetLanguage:$originalText';
              await _cacheService.setTranslationCache(cacheKey, translation);
            }
          }
        }
      ...
      if (kDebugMode) {
        final cacheHitCount = texts.length - uncachedTexts.length;
        if (cacheHitCount > 0) {
          debugPrint('배치 번역 캐시 히트: $cacheHitCount개 (${(cacheHitCount / texts.length * 100).toStringAsFixed(1)}%)');
        }
        if (uncachedTexts.isNotEmpty) {
          debugPrint('배치 번역 완료: ${uncachedTexts.length}개 새로 번역됨');
        }
      }
