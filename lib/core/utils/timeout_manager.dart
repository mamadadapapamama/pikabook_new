...
class TimeoutManager {
  ...
  String getCurrentMessage(String baseMessage) {
    // 운영용: 10초/20초/30초 단계
    // 마지막 단계(30초)에서는 메시지 업데이트 하지 않음 (스낵바에서 처리)
    if (_elapsedSeconds >= 10 && _elapsedSeconds < 20) {
      return '처리 시간이 평소보다 오래 걸리고 있어요. (약 ${_elapsedSeconds}초 경과)';
    } else if (_elapsedSeconds >= 20 && _elapsedSeconds < 30) {
      return '조금 더 기다려 주세요…';
    }
    // 30초 이상일 때는 기본 메시지 유지 (타임아웃시 모달 닫고 스낵바에서 에러 표시)
    return baseMessage;
  }

  /// 단계별 메시지 업데이트가 필요한 시점인지 확인
  bool shouldUpdateMessage() {
    // 운영용: 10초/20초에서만 업데이트 (30초에서는 업데이트 하지 않음)
    // 마지막 단계에서는 메시지 업데이트 하지 않고 타임아웃 콜백에서 스낵바 처리
    return _elapsedSeconds == 10 || _elapsedSeconds == 20;
  }
} 